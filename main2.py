...
class Application(tornado.web.Application):
    def __init__(self):
        handlers = [
            ...
            (r"/clarifications",Clarifications.ClarificationHandler),
...
class Log():
	class LogHandler(BaseHandler,tornado.web.RequestHandler):
		@tornado.web.authenticated
		def get(self):
			...
			log = db.log.find({"uri":"/submission"})

class Clarifications():
	class ClarificationHandler(BaseHandler,tornado.web.RequestHandler):
		@tornado.web.authenticated
		def put(self):
			contests = models.Contest().get_active_contests()
			clarifications = models.Clarification.get_clarifications(contests)
...
class Submission():
    ...
    class SubmissionViewHandler(BaseHandler,tornado.web.RequestHandler):
        ...
        def get(self):
            ...
            submissions = db.submissions.find({"contest":{"$ne":None}}).sort("time",-1)
            page_data = dict(
                title = "Submissions",
                user = self.current_user,
                submissions = list(submissions)
